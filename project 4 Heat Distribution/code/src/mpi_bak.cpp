...
void maintain_fire(float *data, bool* fire_area, int begin, int end) {
    ...
    //int len = (begin-end)*size;
    for (int i = 0; i < (end-begin); i++){
        if (fire_area[begin+i]) data[i] = fire_temp;
    }
}


void maintain_wall(float *data, int rank,int slice) {
    ...
    if (rank==world_size-1){
        for(int i=size;i>0;i--){
            data[slice-i-1]=wall_temp;
        }
    }
    for (int j=0;j<slice-size;j+=size){
        data[j]=wall_temp;
        data[j+size-1]=wall_temp;
    }
}
...
void slave(){
    ...
    //int my_end_row_id = size * (my_rank + 1) / world_size;//no =
    ...
    float* local_data;
    local_data=new float[slices[my_rank]];
    ...
    while (cont) {
        ...
        update(data_odd,local_data,pile[my_rank],pile[my_rank]+slices[my_rank]);
        maintain_fire(local_data,fire_area,pile[my_rank],pile[my_rank]+slices[my_rank]);
        maintain_wall(local_data,my_rank,slices[my_rank]);
        
        // TODO: after computation, send border row to neighbours
        MPI_Gatherv(local_data,slices[my_rank],MPI_FLOAT,data_odd,slices,pile,MPI_FLOAT,0,MPI_COMM_WORLD);
...
void master() {
    ...
    bool* fire_area = new bool[size * size];
    float* local_data;
    local_data=new float[slices[my_rank]*size];
    initialize(data_odd);
